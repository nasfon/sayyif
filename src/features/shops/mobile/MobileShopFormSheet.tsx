...
type ShopFormInput = CreateShopFormValues | EditShopFormValues

interface MobileShopFormSheetProps {
  open: boolean
  mode: 'create' | 'edit'
  shop?: ShopRecord
...
export default function MobileShopFormSheet({
  open,
  mode,
  shop,
  isSubmitting,
  ...
  const {
    ...
    defaultValues: isCreate ? createDefaults : (editDefaults as ShopFormInput),
  })

  useEffect(() => {
    if (open) {
      reset(isCreate ? createDefaults : (editDefaults as ShopFormInput))
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, mode, shop, reset])

  return (
    <BottomSheet open={open} onClose={onClose} title={isCreate ? 'Add Shop' : 'Edit Shop'}>
